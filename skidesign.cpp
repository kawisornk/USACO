...
int main() {
 ...
 if(max <= 17 + min) // max - min <= 17
  ...
  for(short low = min; low <= low_max; ++low, ++high){ // up to O(N) but no more than 83
                                                        int price = 0; //bring all hills to range of [low - high]
   // Add cost below this part
    for(int i = low - 1; i >= 0; --i){ // up to O(N^2) but no more than 83
    if(hills_at_elevation[i]) //there is a hill at height i, which is < low
     price += hills_at_elevation[i] * square_table[low - i];} //bring all hills below low to low
   // Add cost above this part
   for(int i = high + 1; i <= 100; ++i){// up to O(N^2) but no more than 83
    if(hills_at_elevation[i]) //if there is a hill at high
     price += hills_at_elevation[i] * square_table[i - high];} //bring
